firsttemp <- read.table(text = "Year   Jan  Feb  Mar  Apr  May  Jun  Jul  Aug  Sep  Oct  Nov  Dec    J-D D-N    DJF  MAM  JJA  SON  Year
...
#reshape each row as a month
firsttemp <- firsttemp[, 1:13]

newtemp <- data.frame(matrix(t(firsttemp[2:13]), ncol = 1))

newtemp$month <- rep(1:12, 49)
newtemp$year <- rep(1969:2017, each = 12)

names(newtemp) <- c('temp', 'month', 'year')
str(newtemp)
head(newtemp)

firstco2 <- read.table(text = 'site year month value
MLO 1969  8   322.51
MLO 1969  9   321.36
MLO 1969 10   320.74
MLO 1969 11   321.98
...
MLO 2017 10   403.44
MLO 2017 11   405.24
MLO 2017 12   406.69', header =  TRUE, sep = '')

#get rid of site, irrelevant
firstco2 <- firstco2[, 2:4]

head(firstco2)

firstch4 <- read.table(text = 'site year month value
MLO 1983  5  1639.29
MLO 1983  6  1633.53
MLO 1983  7  1633.18
MLO 1983  8  1631.39
...
MLO 2017 10  1882.64
MLO 2017 11  1885.28
MLO 2017 12  1879.49', header = TRUE, sep = '')

firstch4 <- firstch4[, 2:4]

#remind myself of structures for merging
str(newtemp)
str(firstco2)
str(firstch4)
dim(newtemp)
...
climate <- merge(merge(newtemp, firstco2, by = c("year", "month"), all.x = TRUE), firstch4, by = c("year", "month"), all.x = TRUE)
head(climate)
climate <- subset(climate, !is.na(value.x) & !is.na(value.y))
names(climate) <- c("year", "month", "temp", "co2", "methane")

head(climate)
str(climate)

#EDA
#calculate correlations and create scatterplots
cor(climate$co2, climate$temp)
plot(climate$co2, climate$temp)

cor(climate$methane, climate$temp)
plot(climate$methane, climate$temp)

#Response: Global Temp
#Explanatory: C02 (ppm), CH4 (ppb)

#Model: temp = Betao + Beta1CO2 + Beta2CH4 + epsilon
...
#add library to create cool plots (ar stands for applied regression)
library(car)

#component plus residual plots
crPlots(climate_out)
#mine looks different maybe because of IDE?
#yeeaaah it looks the same in Rstudio

avPlots(climate_out)
#This one looks the same, just line is red

#Don't produce both graphs for presentation,
# just choose one

#ANOVA test Ho: no greenhouse gas effect

noeffect <- lm(temp ~ +1, data = climate)
#the +1 indicates that you get the y-intercept, not another variable

#get F-test
anova(noeffect, climate_out)

#since we rejected Ho and concluded that there is a greenhouse effect...
#ttest
summary(climate_out)
...
#create test data based on most recent year (see if our prediction is close to real data..?)
...
#predict 2017 and compare to actual
cbind(climate_test$temp, predict(climate_train_out, newdata = climate_test))
#this table tells us that our predictions (col 2) are on the low end (conservative predictions)

plot(climate_test$temp ~ predict(climate_train_out, newdata = climate_test))

abline(0,1,col='gray')

n2o <- read.table(text = 'year month n2o sd n
1998       1     nan        nan       0
1998       2     nan        nan       0
1998       3     nan        nan       0
1998       4     nan        nan       0
...
hcfc <- read.table(text = 'year month hcfc sd n
1998       1     nan        nan       0
1998       2     nan        nan       0
1998       3     nan        nan       0
1998       4     nan        nan       0
...
2018       7     nan        nan       0
2018       8     nan        nan       0
2018       9     nan        nan       0', header = TRUE)

head(hcfc)
str(hcfc)

sf6 <- read.table(text = 'year month sf6 sd n
...
rownames(climate) <- 1:dim(climate)[1]climate$sf6 <- as.numeric(climate$sf6)str(climate)#calculate corr between temp and new potential greenhouse gasescor(climate$temp, climate$n2o)cor(climate$temp, climate$hcfc)cor(climate$temp, climate$sf6)#model: beta0 + beta1 c02 + beta2 methane + beta3 n2o + #       beat4 hcfc + beta5 sf6 + epsilon, epsilon ~ N(0, sigma^2)climate2_out <- lm(temp ~ co2 + methane + n2o + hcfc + sf6, data = climate)summary(climate2_out)#the coefficients have the wrong sign # (i.e., different than expected)#investigate colinearity - explanatory variables are highly correlated#(co2 has negative estimate)plot(~co2 + methane + n2o + hcfc + sf6, data = climate)#symptoms of colinearity:#wrong sign#model is stat. sig.,#but none of the Ho: Bj = 0 are#prediction is ok#What can we do?#ignore updated model#create a subset model#biased regression (elastic net -> lasso)#because of the collinearity problem, choose a subset regression model#that adds to the greenhouse gases I already know matter (co2, methane)climate2_out_sub <- lm(temp ~ co2 + methane + sf6, data = climate)coef(climate2_out_sub)summary(climate2_out_sub)